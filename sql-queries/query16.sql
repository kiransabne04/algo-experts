...
;with agg_cases as (
	select sum(cases_reported) as counts, extract(month from dates) as mm from covid_cases group by extract(month from dates)
), datasets as (
select 
	counts,
	mm,
	sum(counts) over(order by mm) as running_count
from agg_cases
)
...
select
	*,
	case when mm > 1 
		then cast(round(( counts / lag(running_count, 1) over(order by mm) ) * 100, 1) as varchar)
