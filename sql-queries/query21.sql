...
select
	s.session_id, 
	EXTRACT(EPOCH FROM (session_endtime - session_starttime)) AS session_duration_seconds,
	v.post_id, v.perc_viewed
from user_sessions s
join post_views v
	on s.session_id = v.session_id
), session_posts_percentage as (
	select
	*,
	session_duration_seconds * (perc_viewed / 100 ) as post_view_duration
	from session_posts
	
)

select post_id, sum(post_view_duration) from session_posts_percentage group by post_id;
